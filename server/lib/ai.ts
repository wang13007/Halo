...
type EnergyRecordPreview = {
  deviceName: string;
  energyItemPath: string;
  meterType: string;
  projectName: string;
  sampleDate: string;
  usageKwh: number;
...
const formatEnergyNumber = (value: number) =>
  value.toLocaleString('zh-CN', { maximumFractionDigits: 2 });

const readEnergySummary = (value: unknown) => {
...
const readQueryIssue = (input: HaloChatInput) => {
...
const sumEnergyUsage = (records: EnergyRecordPreview[]) =>
  records.reduce((sum, record) => sum + record.usageKwh, 0);

const groupUsageByDate = (records: EnergyRecordPreview[]) => {
...
const buildFallbackEnergyAnalysis = (
  input: HaloChatInput,
): HaloChatOutput => {
  ...
  const projectName =
    summary?.projectName || sanitizeString(context.project, '未指定项目');
  ...
  lines.push('## 关键结论');
  lines.push(
    `- 本次返回 ${matchedRecordCount} 条记录，覆盖 ${meterCount || records.length} 个表计，总用能 ${formatEnergyNumber(totalUsageKwh)} kWh。`,
  );

  if (topRecords[0]) {
    const topRecordName =
      topRecords[0].deviceName || topRecords[0].energyItemPath || '未命名表计';
    lines.push(
      `- 最高单条记录为 ${topRecordName}，${topRecords[0].sampleDate || '未标注日期'} 用能 ${formatEnergyNumber(topRecords[0].usageKwh)} kWh。`,
    );
  }

  if (highestDate && lowestDate && highestDate.date !== lowestDate.date) {
    const deltaPercent =
      lowestDate.usageKwh > 0
        ? ((highestDate.usageKwh - lowestDate.usageKwh) / lowestDate.usageKwh) * 100
        : 0;
    lines.push(
      `- 按日期汇总，${highestDate.date} 最高 ${formatEnergyNumber(highestDate.usageKwh)} kWh，${lowestDate.date} 最低 ${formatEnergyNumber(lowestDate.usageKwh)} kWh，波动 ${formatEnergyNumber(deltaPercent)}%。`,
    );
  } else if (topShare >= 0.45) {
    lines.push(
      `- Top ${topRecords.length} 表计合计 ${formatEnergyNumber(topUsage)} kWh，占本次结果 ${formatEnergyNumber(topShare * 100)}%，负荷集中度较高。`,
    );
  } else {
  ...
  topRecords.forEach((record, index) => {
    ...
    lines.push(
      `- ${index + 1}. ${recordName}：${formatEnergyNumber(record.usageKwh)} kWh${record.sampleDate ? `（${record.sampleDate}）` : ''}`,
    );
  });
