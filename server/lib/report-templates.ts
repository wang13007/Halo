...
export type ReportPeriod = 'daily' | 'monthly' | 'quarterly' | 'weekly' | 'yearly';
export type ReportScope = 'group' | 'project';
export type ReportTemplateKind = 'single-meter-diagnostic' | 'standard';

export type ReportTemplateSelection = {
  fileName: string;
...
export type ReportDataAvailability = {
  ...
  energyData: boolean;
  equipmentRuntimeData: boolean;
  missingDataLabels: string[];
  multiProjectData: boolean;
  passengerFlowData: boolean;
  publicAreaData: boolean;
  rankingData: boolean;
  singleMeterData: boolean;
  targetData: boolean;
  tenantData: boolean;
...
const standardTemplateFileMap: Record<ReportScope, Record<ReportPeriod, string>> = {
  group: {
...
const singleMeterDiagnosticTemplateFile =
  '单电表用能异常诊断模板_仅基于电表数据.md';
...
const collectRecords = (value: unknown, records: Array<Record<string, unknown>> = []) => {
...
const getPrimaryRecordList = (value: unknown) => {
  if (!value || typeof value !== 'object') {
  ...
  return collectRecords(value).filter((record) =>
    Object.keys(record).some((key) => /orgId|deviceId|sampleTime|totalOne|meterType/i.test(key)),
  );
};
...
const resolveReportPeriod = (input: ReportTemplateInput): ReportPeriod => {
...
const resolveReportScope = (input: ReportTemplateInput): ReportScope => {
  const message = normalizeText(input.message);
...
const looksLikeSingleMeterDiagnostic = (input: ReportTemplateInput) => {
  const message = normalizeText(input.message);
  ...
  return /deviceid|devicename|devicenumber|meterid|metername/.test(
    `${payloadText} ${previewText}`,
  );
};

export const selectReportTemplate = (
  input: ReportTemplateInput,
): ReportTemplateSelection | null => {
...
export const analyzeReportDataAvailability = (
  input: Pick<ReportTemplateInput, 'context' | 'dataPreview' | 'requestPayload'>,
  ...
  const uniqueOrgIds = new Set(
    records.map((record) => normalizeText(record.orgId)).filter(Boolean),
  );
  const hasDeviceFields = records.some(
    (record) =>
      Boolean(record.deviceId) || Boolean(record.deviceName) || Boolean(record.deviceNumber),
  );
  const energyType = normalizeText(input.context?.energyType);

  const availability: ReportDataAvailability = {
    ...
    timeSeriesData:
      records.length > 0 &&
      records.some((record) =>
        ['sampleTime', 'hour', 'day', 'month', 'year', 'totalOne', 'totalTwo'].some(
          (key) => key in record,
        ),
      ),
...
export const buildReportTemplatePromptContext = (input: ReportTemplateInput) => {
